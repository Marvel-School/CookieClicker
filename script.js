...
class Game {
  ...
  setupEventListeners() {
    ...
    [this.saveGameButton, this.loadGameButton, this.resetGameButton].forEach(
      (btn) => btn.addEventListener("mouseover", () => this.playHoverSound())
    );
  ...
  loadGame() {
    ...
    if (confirmLoad) {
      ...
      if (savedGame) {
        ...
        this.soundOn =
          savedGame.soundOn !== undefined ? savedGame.soundOn : true;
        this.updateDisplay();
        this.updateAchievements();
        this.updateGrandmasVisual();
